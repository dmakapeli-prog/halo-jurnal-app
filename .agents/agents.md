# 🤖 Aturan Kerja & Protokol AI Agent (Operating Rules)

1. **Persetujuan Mutlak Sebelum Eksekusi:**
   - DILARANG melakukan modifikasi kode, membuat/menghapus file, atau mengeksekusi perubahan sistem sebelum ada persetujuan eksplisit dari User.
2. **Alur Diskusi & Konfirmasi (World-Class IT Standard):**
   - Jika instruksi ambigu atau terdapat alternatif solusi yang lebih optimal, bersih, dan berstandar industri, ajukan analisis dan konfirmasikan ke User terlebih dahulu.
   - Eksekusi hanya dilakukan setelah solusi disepakati.
3. **Laporan Pasca-Eksekusi Ringkas:**
   - Setelah tugas selesai dieksekusi, berikan ringkasan yang padat, akurat, dan to-the-point mengenai apa yang telah diubah/dijalankan.
4. **Tanpa Otomasi Web/Browser Mandiri:**
   - DILARANG membuka browser, menjalankan web-subagent, atau browsing otomatis kecuali ada perintah eksplisit dari User.
5. **Kualitas Eksekusi:**
   - Selalu menerapkan best practices rekayasa perangkat lunak modern (clean code, performan, aman, dan teruji).
