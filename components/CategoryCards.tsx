...
export default function CategoryCards({ isLoggedIn: initialIsLoggedIn }: CategoryCardsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-[24px]">
      {/* Card 1: Pengaduan */}
      <div
      ...
      {/* Card 4: Inspirasi */}
      <div
        onClick={(e) => handleCategoryClick('inspirasi', e)}
        className="bg-white border border-[#a7d8d0] p-6 md:p-8 rounded-[0.5rem] shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all group cursor-pointer block select-none"
      >
        <div className="w-12 md:w-14 h-12 md:h-14 rounded-[0.75rem] bg-[#ccfbf1] flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 transition-transform">
          <span className="material-symbols-outlined text-[#0d9488] text-2xl md:text-3xl fill-icon">auto_awesome</span>
        </div>
        <h3 className="font-['Libre_Franklin'] text-[20px] md:text-[24px] leading-[28px] md:leading-[32px] font-semibold mb-2">Inspirasi</h3>
        <p className="text-[#574141] mb-4 md:mb-6 font-['Public_Sans'] text-[14px] md:text-[16px] leading-[22px] md:leading-[24px]">
          Ceritakan hal baik yang terjadi di lingkungan Anda untuk menginspirasi warga lain.
        </p>
        <span className="text-[#0d9488] font-['Public_Sans'] text-[14px] leading-[20px] tracking-[0.01em] font-semibold flex items-center gap-2">
          Bagikan Cerita <span className="material-symbols-outlined text-sm">arrow_forward</span>
        </span>
      </div>
    </div>
  )
